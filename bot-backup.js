...
function sendChannelMembershipMessage(chatId) {
    ...
    bot.sendMessage(chatId, membershipMessage, {
        parse_mode: 'Markdown',
...
bot.onText(/\/cancel/, async (msg) => {
    ...
    bot.sendMessage(chatId, '🔄 حالت شما بازنشانی شد. لطفاً یکی از گزینه‌های کیبورد را انتخاب کنید.', {
        parse_mode: 'Markdown',
        ...mainKeyboard
    });
});
...
// Text message handler for keyboard buttons and user input
...
// Handler for non-text messages (photos, stickers, voice messages, etc.)
bot.on('message', (msg) => {
    const chatId = msg.chat.id;
    
    // Only handle non-text messages (skip text messages as they're handled by the 'text' handler)
    if (msg.text) {
        return;
    }
    ...
    try {
        ...
        if (data === 'check_membership') {
            ...
            if (isMember) {
                ...
                try {
                    await bot.editMessageText(config.CHANNEL_MEMBERSHIP.MESSAGES.MEMBERSHIP_VERIFIED, {
                        chat_id: chatId,
                    ...
                    await bot.sendMessage(chatId, config.CHANNEL_MEMBERSHIP.MESSAGES.MEMBERSHIP_VERIFIED, {
                        parse_mode: 'Markdown'
                    });
                }
                ...
                try {
                    await bot.editMessageText(config.CHANNEL_MEMBERSHIP.MESSAGES.MEMBERSHIP_FAILED, {
                        chat_id: chatId,
                        message_id: callbackQuery.message.message_id,
                        parse_mode: 'Markdown',
                        reply_markup: {
        ...
        if (data.startsWith('test_answer_')) {
            ...
            if (state.currentTest && state.currentTest.id === testId) {
                ...
                await bot.editMessageText(testResult, {
                    chat_id: chatId,
                    message_id: callbackQuery.message.message_id,
                    parse_mode: 'Markdown'
                });
